[Markov Chains and HMMs – Towards Data Science](https://towardsdatascience.com/markov-chains-and-hmms-ceaf2c854788)

[Let’s Build a Streaming Data Pipeline – Towards Data Science](https://towardsdatascience.com/lets-build-a-streaming-data-pipeline-e873d671fc57)

[MIT Deep Learning](https://deeplearning.mit.edu/)

[Machine Learning for Beginners: An Introduction to Neural Networks - victorzhou.com](https://victorzhou.com/blog/intro-to-neural-networks/)

[A Simple Explanation of Gini Impurity - victorzhou.com](https://victorzhou.com/blog/gini-impurity/)

[Random Forests for Complete Beginners - victorzhou.com](https://victorzhou.com/blog/intro-to-random-forests/)

[Machine Learning for Everyone :: In simple words. With real-world examples. Yes, again :: vas3k.com](https://vas3k.com/blog/machine_learning/)

[rasbt/deeplearning-models: A collection of various deep learning architectures, models, and tips](https://github.com/rasbt/deeplearning-models)

[The Definitive Guide to Pattern Recognition](https://theappsolutions.com/blog/development/pattern-recognition-guide/)

[How to Label Data — Create ML for Object Detection – Hacker Noon](https://hackernoon.com/how-to-label-data-create-ml-for-object-detection-82043957b5cb)

[Random Forests for Complete Beginners - victorzhou.com](https://victorzhou.com/blog/intro-to-random-forests/)

[Implementing SPADE using fastai – Towards Data Science](https://towardsdatascience.com/implementing-spade-using-fastai-6ad86b94030a)

[Three Popular Clustering Methods and When to Use Each](https://medium.com/predict/three-popular-clustering-methods-and-when-to-use-each-4227c80ba2b6)

[Understanding Decision Trees (once and for all!) 🙌](https://towardsdatascience.com/understanding-decision-trees-once-and-for-all-2d891b1be579)

[Unsupervised Learning of Probably Symmetric Deformable 3D Objects from Images in the Wild - Visual Geometry Group blog](http://www.robots.ox.ac.uk/~vgg/blog/unsupervised-learning-of-probably-symmetric-deformable-3d-objects-from-images-in-the-wild.html?image=022_paint&type=human)

## Kaggle

[Learn Intro to Deep Learning Tutorials | Kaggle](https://www.kaggle.com/learn/intro-to-deep-learning)

## fastai

[fast.ai · Making neural nets uncool again](https://www.fast.ai/)

[Deep Learning for Coders With Fastai and Pytorch Ai Applications Without a Phd by Jeremy Howard, Sylvain Gugger (z-lib.org).pdf](https://app.getpolarized.io/doc/1knH6pYVD2AYGWXFzF65Warge6zph6NQ6a1BdLGmGEAbz7pcfo#?page=19&target=12RLf6A7k1&pos=bottom&n=94969)

[bradleybossard/fastbook: Draft of the fastai book](https://github.com/bradleybossard/fastbook)

[Welcome to fastai | fastai](https://docs.fast.ai/)

[fastai: A Layered API for Deep Learning](https://app.getpolarized.io/doc/12AW5aGAguHoWsAPZE64FiVqJCRYrUhN5d3LT7biNViGLiaRVT7#?page=3&target=12rRxhMq28&pos=bottom&n=58516)

[Fast AI Video Viewer](https://course.fast.ai/videos/?lesson=1)

## An Intro to Statistical Learning

[An Introduction to Statistical Learning](https://app.getpolarized.io/doc/1DETP75FyxGqWW73uhoJra7gUm4MUksaUUHXYvKn6UooQwpa4j#?page=14&target=1jumUApYRt&pos=bottom&n=50028)

[hardikkamboj/An-Introduction-to-Statistical-Learning: This repository contains the exercises and its solution contained in the book "An Introduction to Statistical Learning" in python.](https://github.com/hardikkamboj/An-Introduction-to-Statistical-Learning)

## Hands-On Machine Learning with Scikit-Learn and TensorFlow

[Hands-On Machine Learning with Scikit-Learn and TensorFlow - Hands-On Machine Learning with Scikit-Learn and TensorFlow](https://learning.oreilly.com/library/view/hands-on-machine-learning/9781491962282/titlepage01.html)

[ageron/handson-ml: A series of Jupyter notebooks that walk you through the fundamentals of Machine Learning and Deep Learning in python using Scikit-Learn and TensorFlow.](https://github.com/ageron/handson-ml)

## Deepmind

[deepmind/deepmind-research: This repository contains implementations and illustrative code to accompany DeepMind publications](https://github.com/deepmind/deepmind-research)

## Deeplizard

[Deep Learning playlist overview & Machine Learning intro - deeplizard](https://deeplizard.com/learn/video/gZmobeGL0Yg)


## Serokell

[A Guide to Deep Learning and Neural Networks](https://serokell.io/blog/deep-learning-and-neural-network-guide)

## MIT Deep Learning

[MIT Introduction to Deep Learning | 6.S191 - YouTube](https://www.youtube.com/watch?v=njKP3FqW3Sk&feature=emb_logo&t=41s)

[Deep Learning State of the Art (2020) | MIT Deep Learning Series - YouTube](https://www.youtube.com/watch?v=0VH1Lim8gL8&feature=emb_logo&t=28s)

## Google

[Machine Learning Crash Course  |  Google Developers](https://developers.google.com/machine-learning/crash-course)

[Tracking the Performance of Your Machine Learning Models With MLflow - Mutt Data Blog](https://muttdata.ai/blog/2021/02/12/ml-flow.html)
